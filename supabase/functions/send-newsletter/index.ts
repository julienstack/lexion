...
function generateNewsletterHtml(
    ...
    const footer = config.footer_text || 'Lexion - Vereinsverwaltung';
...
<tr>
  <td style="padding: 12px 0; border-bottom: 1px solid #eee;">
    ...
    const logoHtml = config.logo_url
        ...
        : `<span style="font-size: 24px; font-weight: bold; color: ${color};">Lexion</span>`;
        ...
        if (config.smtp_host) {
            ...
            if (test_email) {
                await transporter.sendMail({
                    from: config.smtp_from || '"Lexion" <newsletter@lexion.dev>',
                    to: test_email,
                    subject,
                    html: emailHtml
                });
